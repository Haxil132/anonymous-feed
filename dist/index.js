...
async function addLike(like) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const existing = await db.select().from(likes).where(sql`${likes.postId} = ${like.postId} AND ${likes.ipAddress} = ${like.ipAddress}`).limit(1);
...
async function getLikeCount(postId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.select({ count: sql`count(*)` }).from(likes).where(eq(likes.postId, postId));
  return result[0]?.count ?? 0;
}
async function hasUserLikedPost(postId, ipAddress) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.select().from(likes).where(sql`${likes.postId} = ${postId} AND ${likes.ipAddress} = ${ipAddress}`).limit(1);
...
async function getCommentCount(postId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.select({ count: sql`count(*)` }).from(comments).where(eq(comments.postId, postId));
  return result[0]?.count ?? 0;
}
...
var LOG_FILE = join(process.cwd(), "ip-logs.txt");
function logIP(ipAddress, action, details) {
  const timestamp2 = (/* @__PURE__ */ new Date()).toISOString();
  const logEntry = `[${timestamp2}] IP: ${ipAddress} | Action: ${action}${details ? ` | ${details}` : ""}
`;
  try {
  ...
  posts: router({
    create: publicProcedure.input(z2.object({
      ...
      if (input.mediaFile) {
        ...
        mediaUrl = result.url;
        mediaType = input.mediaFile.type.startsWith("image/") ? "image" : input.mediaFile.type.startsWith("video/") ? "video" : input.mediaFile.type.startsWith("audio/") ? "audio" : "file";
      }
      await createPost({
        title: input.title,
...
IP: ${ipAddress}`
    ...
    list: publicProcedure.input(z2.object({
      ...
      const ipAddress = getClientIP(ctx.req);
      const postsList = await getPosts(input.limit, input.offset);
      const enrichedPosts = await Promise.all(
        postsList.map(async (post) => {
          const [likeCount, commentCount, hasLiked] = await Promise.all([
            getLikeCount(post.id),
            getCommentCount(post.id),
            hasUserLikedPost(post.id, ipAddress)
          ]);
          return {
            ...post,
            likeCount,
  ...
  likes: router({
    toggle: publicProcedure.input(z2.object({
      postId: z2.number()
    })).mutation(async ({ input, ctx }) => {
      const ipAddress = getClientIP(ctx.req);
      ...
      const likeCount = await getLikeCount(input.postId);
      return {
        success: true,
  ...
  comments: router({
    ...
    list: publicProcedure.input(z2.object({
      ...
      const commentsList = await getCommentsByPostId(input.postId);
      return commentsList;
    })
  })
});
